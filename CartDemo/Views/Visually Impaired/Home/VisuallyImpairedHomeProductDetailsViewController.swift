//
//  VisuallyImpairedHomeProductDetailsViewController.swift
//  CartDemo
//
...
class VisuallyImpairedHomeProductDetailsViewController: UIViewController {
    ...
    @IBOutlet weak var visualProductCategory: UILabel!
    @IBOutlet weak var visualProductDescription: UILabel!
    @IBOutlet weak var visualProductQty: UILabel!
    @IBOutlet weak var visualWishButton: UIButton!
    @IBOutlet weak var visualAddToCart: UIButton!
    
    // MARK: Variables
    var selectedVisualProduct : VisualProductHolder!
    var selectedProductID: Int = 0
    
    var visualHomeProductQtyCount: Int = 0
    ...
    override func viewDidLoad() {
        ...
        // make image autoload
        let productImageURL = selectedVisualProduct.visualProductImage
        ...
        let task = URLSession.shared.dataTask(with: urlRequest)
        {
            ...
            if(error == nil)
            {
                ...
                DispatchQueue.main.async { // [self] in
                    self.visualProductImage.image = imageProd
                }
            }
        }
        task.resume()
        
        visualProductName.text = selectedVisualProduct.visualProductName
        visualProductPrice.text = "$" + (String)(selectedVisualProduct.visualProductPrice)
        visualProductCategory.text = selectedVisualProduct.visualProductCategory
        visualProductDescription.text = selectedVisualProduct.visualProductDescription
        
        // for insert to cart variables
        selectedProductID = selectedVisualProduct.visualProductID
        productPriceHolder = selectedVisualProduct.visualProductPrice
        
        visualHomeProductQtyCount = 0
    }
    
    
    @IBAction func visualAddToCartButton(_ sender: UIButton) {
        ...
        if (userHasCart() == false){
            ...
            if(Int(visualProductQty.text!) == 0){
                ...
                if sqlite3_prepare_v2(dbQueue, insertCartProductString, -1, &insertCartProductStatementQuery, nil) == SQLITE_OK {
                    ...
                    sqlite3_bind_int(insertCartProductStatementQuery, 3, Int32(visualHomeProductQtyCount))
            ...
            if(Int(visualProductQty.text!) == 0){
                ...
                if (productIsInCart() == false){
                    ...
                    let updateCartProductString = "UPDATE CartProduct SET cartProductQty = \(visualHomeProductQtyCount) WHERE cartID = \(currentCartID) AND productID = \(selectedProductID)"
    ...
    @IBAction func visualAddWish(_ sender: UIButton) {
        ...
        visualWishButton.setImage(UIImage(systemName: "heart.fill"), for: .normal)
        ...
        if(checkProductIsInWishList() == false){
            ...
            if sqlite3_prepare_v2(dbQueue, insertWishProduct, -1, &insertWishProductQuery, nil) == SQLITE_OK {
                ...
                let wishProductName = visualProductName.text!
                let wishProductDescription = visualProductDescription.text!
                let wishProductCategory = visualProductCategory.text!
                let wishProductPrice = selectedVisualProduct.visualProductPrice!
                let wishProductImage = selectedVisualProduct.visualProductImage!
    ...
    @IBAction func visualDecrementQtyButton(_ sender: UIButton) {
        if visualHomeProductQtyCount != 0{
            visualHomeProductQtyCount -= 1
            self.visualProductQty.text = (String)(visualHomeProductQtyCount)
        }
        else{
            visualHomeProductQtyCount = 0
            self.visualProductQty.text = (String)(visualHomeProductQtyCount)
        ...
        visualAddToCart.setTitle("Add \(visualHomeProductQtyCount) to cart", for: .normal)
    }
    
    @IBAction func visualIncrementQtyButton(_ sender: UIButton) {
        if visualHomeProductQtyCount == 0{
            visualHomeProductQtyCount += 1
            self.visualProductQty.text = (String)(visualHomeProductQtyCount)
        }
