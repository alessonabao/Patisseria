//
//  VisuallyImpairedHomeViewController.swift
//  CartDemo
//
...
class VisuallyImpairedHomeViewController: UIViewController, UITableViewDataSource, UITableViewDelegate{

    // MARK: Variables
    var products = [VisualProductHolder]()    // array to show all products
    
    // MARK: DB variables
    let SQLITE_TRANSIENT = unsafeBitCast(-1, to: sqlite3_destructor_type.self)
    
    // MARK: Outlets
    @IBOutlet weak var visualHomeTableView: UITableView!
    @IBOutlet weak var visualSearchTextField: UITextField!
    
    override func viewDidLoad() {
        super.viewDidLoad()

        // Do any additional setup after loading the view.
        visualHomeTableView.dataSource = self
        visualHomeTableView.delegate = self
        
        loadSavedProducts()
    }
    
    // MARK: Actions
    @IBAction func visualSearchProduct(_ sender: UIButton) {
        
        let searchedProduct = visualSearchTextField.text ?? ""
        ...
        if searchIsValid(){
            ...
            if sqlite3_prepare_v2(dbQueue, selectSearchedProductStatementString, -1, &selectSearchedProductStatementQuery, nil) == SQLITE_OK {
                while sqlite3_step(selectSearchedProductStatementQuery) == SQLITE_ROW{
                    ...
                    let savedProduct = VisualProductHolder(
                        visualProductID: productID,
                        visualProductName: productName,
                        visualProductDescription: productDescription,
                        visualProductCategory: productCategory,
...
//                print(showData)
                ...
                visualHomeTableView.reloadData()
            }
        }
        else{
    ...
    @IBAction func loadAllSort(_ sender: UIButton) {
        loadSavedProducts()
    }
    
    @IBAction func loadNewSort(_ sender: UIButton) {
        popularSort()
    }
    
    @IBAction func loadPopularSort(_ sender: UIButton) {
        newSort()
    }
    
    @IBAction func loadGlutenSort(_ sender: UIButton) {
        glutenFreeSort()
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = visualHomeTableView.dequeueReusableCell(withIdentifier: "visualHomeCell", for: indexPath) as! VisuallyImpairedHomeTableViewCell
        
        let thisProduct = products[indexPath.row]
        
        cell.visualProductName.text = thisProduct.visualProductName
        cell.visualProductPrice.text = "$" + (String)(thisProduct.visualProductPrice)
        
        // *****************************LOAD PRODUCT IMG URL*****************************
        let urlText = thisProduct.visualProductImage
            ...
            if(error == nil){
                do{
                    ...
                    DispatchQueue.main.async { // [self] in
                        cell.visualProductImage.image = imageProd
                    }
                }
                catch{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        self.performSegue(withIdentifier: "visualProductDetailSegue", sender: self)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if(segue.identifier == "visualProductDetailSegue"){
            let indexPath = self.visualHomeTableView.indexPathForSelectedRow!
            let tableViewDetail = segue.destination as? VisuallyImpairedHomeProductDetailsViewController

            let selectedProduct = products[indexPath.row]
            tableViewDetail!.selectedVisualProduct = selectedProduct
            self.visualHomeTableView.deselectRow(at: indexPath, animated: true)
        }
    }
...
//                print(showData)
    ...
    func searchIsValid() -> Bool{
        ...
        guard let searchedProduct = visualSearchTextField.text, !searchedProduct.isEmpty else {
            return false
        }
    ...
    func popularSort(){
        ...
        if sqlite3_prepare_v2(dbQueue, popularSortStringQuery, -1, &popularSortStatementQuery, nil) == SQLITE_OK {
            while sqlite3_step(popularSortStatementQuery) == SQLITE_ROW {
                ...
                let popularProduct = VisualProductHolder(
                    visualProductID: productID,
                    visualProductName: productName,
                    visualProductDescription: productDescription,
                    visualProductCategory: productCategory,
    ...
    func newSort(){
        ...
        if sqlite3_prepare_v2(dbQueue, newSortStringQuery, -1, &newSortStatementQuery, nil) == SQLITE_OK {
            while sqlite3_step(newSortStatementQuery) == SQLITE_ROW {
                ...
                let newProduct = VisualProductHolder(
                    visualProductID: productID,
                    visualProductName: productName,
                    visualProductDescription: productDescription,
                    visualProductCategory: productCategory,
    ...
    func glutenFreeSort(){
        ...
        if sqlite3_prepare_v2(dbQueue, glutenFreeSortStringQuery, -1, &glutenFreeSortStatementQuery, nil) == SQLITE_OK {
            while sqlite3_step(glutenFreeSortStatementQuery) == SQLITE_ROW {
                ...
                let glutenFreeProduct = VisualProductHolder(
                    visualProductID: productID,
                    visualProductName: productName,
                    visualProductDescription: productDescription,
                    visualProductCategory: productCategory,
